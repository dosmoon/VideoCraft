...
| 🟡 P2 | [x] | AI 取消传播 wiring (X2) | 全链路上线。**翻译**：translate_srt 单工具 + manifest step3_translate tri-state 按钮，batch 边界 `throw_if_cancelled` 抛 `AIError(Kind.CANCELLED)`。**ASR**（Lemonfox）：每 attempt 用 `requests.Session()`，注册 `session.close` 为 abort_cb；`_ProgressFile.read` 也查 token 让上传秒停；session 关闭后 ConnectionError 被识别为 cancellation。**Pack**（gemini / openai_compat / claude_code）：openai client.close + gemini `_safe_close`（沿 SDK schema 寻 close）+ claude_code 改 Popen 注册 `terminate()`；router._call_json 也透传 cancel_token；`_complete_json_by_tier` 不再对 CANCELLED 自动 fall-back。**TTS**（fish_audio）：cancel_token 折进现有 should_cancel 谓词，每 chunk 检查。Workbench `_CANCELLABLE_STEPS = {step2_asr, step3_translate, step5_pack}`，三个 worker 全接 token，CANCELLED → status pending 不出错误对话框。Cancel 延迟现 <1s（之前 30s-10min）。spec 见 [docs/design/04-ai-router.md](docs/design/04-ai-router.md) "取消传播（X2）" 章节。commits c43ba2e / b796fc5 / 1121de6 / 621ec16 |
...
| 完成时间 | 功能 | 备注 |
|---------|------|------|
| 2026-05-01 | AI 取消传播 X2 part 2（ASR / pack / TTS HTTP 秒停）| 把 part 1 留下的 ASR / pack / TTS 三条 single-call 路径补全到 abort-callback 级取消。**Lemonfox**：每 attempt 起 `requests.Session()`，cancel_token 注册 `session.close`，关闭后阻塞的 POST 收到 ConnectionError → 识别为 CANCELLED；`_ProgressFile.read` 也查 token 让上传过程随 chunk 秒停。**openai_compat**：`OpenAI(...)` client 注册 close → 关 httpx 池 → 在途请求 APIConnectionError → 识别为 CANCELLED。**gemini**（google.genai）：`_safe_close` 沿 close / _api_client.close / _client.close 找一个调用，SDK schema 跨版本不一所以兜底失败静默。**claude_code**：subprocess.run 改成 Popen + communicate，注册 `proc.terminate()`。Router `_call_json` 全透传 cancel_token；`_complete_json_by_tier` 在 CANCELLED 时不 fall-back（用户要 OUT 不要换 provider）。**TTS**：fish_audio 把 cancel_token 折进 should_cancel 预测器，per-chunk 检查；text2video 老 `_stop_flag` 路径不动。Workbench `_CANCELLABLE_STEPS` 加 step2_asr + step5_pack，两个 worker 接 token + CANCELLED → pending 状态。Cancel 延迟从 30s-10min 降到 <1s。commits 1121de6 / 621ec16 |
