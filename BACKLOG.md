...
| 优先级 | 状态 | 功能 | 说明 |
|--------|------|------|------|
| 🔴 P1 | [ ] | 拆出独立仓库 `easy-ytdlp`（MIT），从 VideoCraft 移除 yt-dlp 模块 | yt-dlp 在 VideoCraft 里既增加打包体积、又把 VideoCraft 跟 YouTube ToS 灰区绑定，不适合作为商业产品/正式发行品的一部分推广。方案：新建独立 repo `dosmoon/easy-ytdlp`（MIT），把 `src/tools/youtube/*` + JS runtime 处理 + 输出路径偏好 + i18n 子集 + cookies 处理整套迁过去，独立 GUI（Tk 或 Electron 都行，按"小工具"形态）；VideoCraft 主仓清理 youtube tool / requirements 里的 yt-dlp / 菜单项 / i18n 残留。两个产品维持独立发版节奏。**Why:** YouTube 下载是 ToS 灰区，不适合跟 VideoCraft 商业层绑定；同时 yt-dlp 自身有 JS runtime 等系统级依赖，独立工具形态更适合普通用户使用。**How to apply:** 暂不动手；本条只占位，等当前内嵌 AI / AI Console 重构告一段落后启动 |
