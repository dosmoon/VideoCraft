...
- **Preview ≡ render 的渲染前数据变换必须是单一纯函数**——任何"既影响烧录又影响 preview"的转换（subtitle 自动换行 / hook/outro wrap / overlay 文本布局……）只能存在一处实现，两条路径都调它；不允许"render 跑一份、preview 跑另一份"，更不允许 preview 跳过变换走原始数据。违反这条 = silent preview≠render 分裂，是这个引擎里最严重的回归类型

### Preview ≡ render 不变量的实例

| 变换 | 单一实现位置 | render caller | preview caller |
|---|---|---|---|
| Subtitle 自动换行（按 aspect/fontsize 算 max_chars） | `render.wrap_subtitle_elements()` | `_subtitle_elements_to_temp_srt()` | `CompositionPreview.set_timeline()` 子幕分支 |
| Hook/outro 文本换行 | `text_layout.wrap_hook_outro()` | `drawtext_helpers.drawtext_filter()` | （JS 端按 lines 直渲，由 Python 预算） |
| Watermark 文本换行 | `text_layout.wrap_overlay_text()` | `primitives/text_watermark.build_drawtext()` | （同上） |

新增任何"渲染前最后变换"必须按这个模式注册一个共享纯函数，**不允许在 preview/render 各自就地实现**。回归测试模板见 `tests/composition/test_pr4_timeline_render_e2e.py::test_preview_and_render_share_same_subtitle_wrap`——给一段超过单行宽度的 fixture，两条路径各跑一遍，断言输出文本列表严格相等。
