...
> **2026-04 之前的旧布局**：单表 (provider, model) × task 单选矩阵，Gemini 模型多了之后行数膨胀；矩阵已废弃，逻辑迁到上述两段式。Phase 2 仍计划把"Providers" 段进一步卡片化 + 模型 picker 对话框（解决 list_models 一次性塞 20+ 模型到 textarea 的痛点），见 BACKLOG。
...
| AI 控制台 | ✅ 三 tab：Provider & 路由（两段式：Task Routing 下拉 + Providers 列表）/ Prompts / 统计 | Providers 段卡片化 + 模型 picker 对话框（Phase 2）；调用费用估算 / 错误率列 |
