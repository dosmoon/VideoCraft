...
> **关联**:[`electron-migration-design.md`](electron-migration-design.md)(UI 外壳迁移;其"渲染后端留 Python ffmpeg"一节被本文 §4 取代)。
...
(取代 [`electron-migration-design.md`](electron-migration-design.md) §4"渲染后端留 Python ffmpeg"一节。)
...
| [01-architecture](../design/01-architecture.md) 单进程无 IPC | 已由 electron-migration-design 标记 supersede |
| [`electron-migration-design.md`](electron-migration-design.md) | **互补 + 修订**:本文定数据模型 + 渲染引擎;迁移文档定 UI 外壳 + IPC。迁移文档 §4"渲染后端留 ffmpeg"由本文 §4"自建 GPU 合成器"取代——迁移文档需同步更新 |
...
## 音频 + compositing 实现状态(2026-05-30,commit `e9f8e72`)
