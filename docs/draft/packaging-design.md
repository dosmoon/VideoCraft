...
- **hiddenimports**:`methods/__init__.load_plugins()` 动态 import 插件 + provider 域,PyInstaller 静态分析抓不全 → 显式列(creations/materials/core.ai providers/`core_rpc.methods.*`)。
...
| 依赖 | 策略 | 依据 |
|---|---|---|
| ffmpeg / ffprobe | **随包**(`extraResources`) | 视频=核心功能,非 AI,不能被下载门槛堵死([[feedback_no_forced_downloads]] 的"非 AI 不被 AI gate");`ffmpeg.ts` 已优先 `resourcesPath`。 |
