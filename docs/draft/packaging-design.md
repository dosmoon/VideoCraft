...
- **⚠️ winCodeSign 符号链接坑(CI 必读)**:electron-builder 在 Windows eager 解压 `winCodeSign`(签名工具,归档含 macOS `.dylib` 符号链接);**非 admin / 无 Developer Mode 的机器** 7za 建符号链接失败(`SeCreateSymbolicLinkPrivilege` 缺)→ 整 build 挂,`--dir` 也不能幸免。**绕法 = `win.signAndEditExecutable:false`**(关签名 + rcedit 编辑 → 不再需要 winCodeSign)。代价:exe 用默认 Electron 图标、不签名。**✅ 已解(2026-06-09)**:CI workflow(§8 step 8)在 GitHub windows runner 上就地把 yml `signAndEditExecutable: false` flip 成 true → rcedit 嵌品牌图标(runner 是 admin,有符号链接权限);本机 yml 默认仍关。真 Authenticode 签名仍待证书(§10)。
...
| userData | `resolve(__dirname,"../../user_data")`(repo-local) | **install-local:`<exeDir>/user_data`**(NSIS `perMachine:false` 装 `%LOCALAPPDATA%\Programs\VideoCraft`,该处可写)。经 `VC_USER_DATA` 注入统一给 Python sidecar(§5.3)。**~~待定~~ 已定(2026-06-05)**:见下「user_data 的更新存活」。 |
...
### user_data 的更新存活(2026-06-05,解决原 §4/§9 待定 + P1「用户数据绿色化」)

原「便携 zip vs NSIS 可写根」的待定就此定案 = **保留 portable-data 铁律(数据恒在 exe 旁 `<exeDir>/user_data`),让 NSIS 学会不删它**——而非退守 `%LOCALAPPDATA%` 例外。两处真 bug 修掉:

1. **keys 归位 user_data**:`core/ai/config.py` 的 `keys_dir()` 是唯一还 `__file__`-relative 的用户数据写点 → 打包态落在 sealed `resources/_internal/keys`(在 `$INSTDIR` 内、每次重装即抹)。改为冻结态走 `core.user_data.path("keys")` = `<exeDir>/user_data/keys`(dev 仍 `<repo>/keys`,不动开发者真实 key,无凭据迁移风险)。providers.json + 所有 `.key` 随之进 user_data。
2. **NSIS 保留宏**:electron-builder 默认卸载器 `RMDir /r $INSTDIR`,且**更新流程会先跑旧卸载器再解包新版** → 不修则每次更新/重装都连 `$INSTDIR\user_data`(模型/嵌入 AI/settings/key)一起删。新增 `desktop/build/installer.nsh`(electron-builder 自动从 buildResources 收 `installer.nsh`,无需改 yml)定义 `customRemoveFiles`:把 user_data 挪到同卷兄弟目录(瞬时 metadata move,GB 模型不拷贝)→ 清 `$INSTDIR` → 挪回。

**⚠️ 只保护「带此宏的 build」往后的更新**:第一次覆盖装在 pre-fix 旧版之上时,跑的是**旧卸载器(无保留宏)**,仍会抹 → fixed build 必须先干净装一次。

`models_dir`(可被 `providers.json` override 指到外置盘)独立于此:外置模型本就不在 `$INSTDIR`,天然不受影响。
...
- 顺序:`build_sidecar.ps1`(uv sync 冻结 sidecar)→ `fetch_ffmpeg.ps1`(拉 pinned ffmpeg 进 resources,幂等)→ `generate_build_info.ps1`(产 `resources/build-info.json`,electron-builder extraResources 必需;漏跑则 build 报缺文件)→ `pnpm build:win`(electron-vite build + electron-builder)。
- CI:✅ `.github/workflows/build-windows.yml`(dispatch + `v*` tag;上述四步 + signAndEditExecutable flip + 传 artifact)。`BUILD_NUMBER=github.run_number` 注入 → Windows FileVersion `<version>.<run_number>`。版本号规则见 [`../versioning.md`](../versioning.md)。
...
8. **打磨**:图标/版本/产品名;`disable-gpu-sandbox` 复审;CI(✅ 见下);最后重打包做打包态终验(含 §5.3 的 AI/GPU 安装真机验 + ffmpeg 1080p 合并 GUI 验)。
   - ✅ **CI workflow(2026-06-09)**:`.github/workflows/build-windows.yml`——`windows-latest`(admin → 有 winCodeSign 符号链接权限)上复现全链路(setup-uv → `build_sidecar.ps1` → `fetch_ffmpeg.ps1` → pnpm `electron-vite build` + `electron-builder --win`),**CI 就地把 checkout 的 yml `signAndEditExecutable: false` flip 成 true**(不用 `-c.win...=true` CLI override——那个点号短形式被 yargs 解析成 config 文件路径 ENOENT)→ rcedit 嵌品牌图标。`workflow_dispatch` + `v*` tag 触发;installer 传 artifact,`--publish never`(不自动发 Release)。**真 Authenticode 签名仍 deferred**(无证书;见 §10)。

---

## 9. 待决 / 风险

- ✅ **userData 可写根 + 更新存活**(§4/§5.3):step 6 NSIS `perMachine:false` 装 `%LOCALAPPDATA%` 可写;step 7 `VC_USER_DATA` 注入让 Python sidecar user_data 与 Electron 统一落 `<exeDir>/user_data`。**更新存活非自动**——靠 2026-06-05 新增的 `desktop/build/installer.nsh` `customRemoveFiles` 宏在卸载时挪开 user_data(默认 `RMDir /r $INSTDIR` 会连数据删),配套 keys 归位 `user_data/keys`。详见 §4「user_data 的更新存活」。便携 zip 另算但 NSIS per-user 已够。
...
- ✅ **版本线**:`package.json` / `pyproject.toml` 均已对齐 `0.3.5`。**版本号怎么涨、单一源、tag 规则 = [`../versioning.md`](../versioning.md)**(权威)。

---

## 10. 发布 checklist(切发布版前)

> 版本号怎么定(涨哪一位、单一源、tag `vX.Y.Z` 规则)见 [`../versioning.md`](../versioning.md)。
