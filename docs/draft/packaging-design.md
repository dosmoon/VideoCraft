...
2. **bundle pip 进冻结包**:`requirements-base.txt` 加 `pip==26.1`(uv venv 默认无 pip);`core_rpc.spec` 加 `collect_submodules("pip")` + `collect_data_files("pip")`。
...
4. **嵌入 AI 装包 = `core/embedded_ai_install.py`**(镜像 `gpu_install`):装 `faster-whisper==1.2.1` + `llama-cpp-python==0.3.22`(CPU,`--extra-index-url …/whl/cpu`)到 py-extra;版本锁步 requirements.txt。
...
**⚠️ 打包态终验(未做,GUI/真机盲区)**:① 冻结 `core_rpc.exe --vc-pip install` 真能装出 wheel(尤其 llama-cpp-python CPU wheel 从 abetlen 索引、ctranslate2 binary wheel 在冻结 interpreter tag 下匹配)② 装后 ASR/本地 LLM 真能跑(py-extra prepend + warmup import 链)③ CUDA wheels 装 py-extra 后 `nvidia/*/bin` DLL 解析。需重打包后真机走一遍。
...
8. **打磨**:图标/版本/产品名;`disable-gpu-sandbox` 复审;ffmpeg pinned 随包(step 4 defer);CI;最后重打包做打包态终验(含 §5.3 的 AI/GPU 安装真机验)。
