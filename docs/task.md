...
**下一步(迁移)**:**Phase B1 已完成**(2026-06-01,commit `b1ea0ad`→`ccdb1de`)——TS 素材数据层全落:`materials/news_video/{schema,paths,model}.ts` + `project.material_instance_dir` RPC,177 vitest 全绿,纯数据层无能力依赖(business actions 归 B2)。**接力下一步 = B2 能力网关**(`core_rpc/methods/capability.py` 路径式 job:acquire_source/asr/translate/analyze/ai_fill/probe/subtitle_check/quick_fix/save_chapters,复用 `_jobs_util.py`;`subtitle_pipeline` 加 `*_paths` 消 ADR-0005 越界 TODO;`ai_fill.extract` 移进 `core/`)。**或** A6 退役创作 Python(仍卡 **P2 Tk 退役**:clip Tk 还 import `clip/config.py`)。进度勾选权威 = [`adr-0008-migration-tasks.md`](draft/adr-0008-migration-tasks.md)。
