...
> **🚩 重大架构转向已拍板(续 37,未提交)= 三个本体收敛为纯 TS 插件,Python 退成 plugin-agnostic 能力网关。** 起因:为 news_desk 恢复 publish.md(续 36)暴露"双语插件不可维护"。**权威方案已并入 `docs/draft/electron-migration-design.md` 顶部「🚩 架构转向」节(预定 ADR-0008)**——先读它。落地 = 迁移工作计划新增 **P2.5**(Phase A 创作全 TS → Phase B 素材全 TS + `capability.*` 能力网关),**前提 = 先完成 P2 Tk 退役**(clip Tk 仍依赖 `clip/config.py`)。**当前状态 = 方案刚并入文档,待用户过目,尚未动代码。**
...
## ▶ 续 37(2026-06-01,架构转向拍板:插件全 TS / Python 能力网关 — 仅文档,未提交)
...
- **本轮产出 = 仅文档**:完整方案已并入 `docs/draft/electron-migration-design.md` 顶部「🚩 架构转向」节(权威,**预定 ADR-0008**);并打了 supersede 标记(3 个「Python 业务面」节 ⛔ / §2.2 RPC面 / §2.3 内存所有者 / 工作计划新增 P2.5 / P6 ADR-0008 改写为本转向)。**未动任何代码。** 详细 C1~C7 地基 + Phase A/B 步骤 + 风险 + 必读文件 全在该节(plan 文件 `~/.claude/plans/electron-python-ticklish-mountain.md` 是同内容副本,以 design 文档为准)。
- **接力下一步**:用户过目「架构转向」节 → commit 文档(连同续 36 两笔)→ 起 ADR-0008 → 进 P2(Tk 退役)→ P2.5 Phase A。**工作纪律**:per-port 取注入式 `fs` 接口供 vitest;每步 build-green、TS 替代测好前绝不删对应 Python;`writeJson` 原子复刻 Python `*.save()` 保 golden;`assertInProject` 守路径安全([[feedback_portable_data]])。
