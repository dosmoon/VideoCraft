...
**下一步(迁移)**:**Phase B1 + B2 后端已完成**(2026-06-01,commit `b1ea0ad`→`0107420`)。**B1** = TS 素材数据层(`materials/news_video/{schema,paths,model}.ts` + `project.material_instance_dir` RPC,177 vitest)。**B2** = 能力网关 `core_rpc/methods/capability.py`(路径式 job acquire_source/asr/translate/analyze + sync subtitle_check/quick_fix/save_chapters + 通用 `llm_extract`;`subtitle_pipeline` 加 plugin-free `*_paths` 消 ADR-0005 越界;190 pytest 全绿)。**🚩 ai_fill 用户拍板=纯通用**:capability 零领域知识,只 `llm_extract({prompt,schema,task})`;新闻 prompt+15 字段 schema 移 TS(归 B3/B4)。**接力下一步 = B3 接线素材工作台**(`workbenches/material/*` 改走 TS `NewsVideoModel` + 经 `runJob.ts` 调 `capability.*`;`client.ts` 把 `material.*`→`capability.*`;news prompt 模板移 TS;加 project meta 写 RPC 持久化 ASR 检出语言)。**或** A6 退役创作 Python(仍卡 **P2 Tk 退役**)。进度勾选权威 = [`adr-0008-migration-tasks.md`](draft/adr-0008-migration-tasks.md)。
