...
- **下一步 = ⚠️真机验**(headless 盲区,必须你跑):真机装 `VideoCraft-0.3.5-setup.exe` 或直接跑 `release/win-unpacked/VideoCraft.exe`,验启动 + WebGPU 合成 + 导出 + sidecar(`resources/sidecar/core_rpc.exe`)连上 + AI job 在缺 faster-whisper/llama 下优雅报错不崩。之后 = step 7 嵌入 AI opt-in 装 `py-extra`(§5.3,新代码)+ step 8 打磨(图标/签名/CI/ffmpeg pinned 随包)。完整计划 packaging-design.md §8。
