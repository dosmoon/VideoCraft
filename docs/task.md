...
- **⚠️ 剩 GUI 真机终验(用户)**:① GUI 点安装确认顺(机制已命令行钉死)② 装后 **ASR/本地 LLM 真能跑**(加载模型+推理,需源视频)③ CUDA wheels 装 py-extra 后 nvidia DLL 解析。
- **下一步 = step 8 打磨**(图标/签名/ffmpeg pinned/CI)+ 上面 GUI 剩余终验。两个 commit(`4ef3956`/`0c4490a`)**未 push**,视用户意([[feedback_external_actions]])。
