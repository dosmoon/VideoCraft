...
- **打包态验证(命令行,本会话做完)**:重打包 sidecar(`build_sidecar.ps1`,加 `--clear`)+ `pnpm build:win` → `VideoCraft-0.3.5-setup.exe` **130.6MB**(NSIS per-user)+ `win-unpacked/resources/sidecar/core_rpc.exe`。**冻结 `--vc-pip install` 决定性验证通过**:打包产物的 `core_rpc.exe --vc-pip install six --target X --only-binary :all:` → exit 0、six.py 落 X(dev 树 + win-unpacked 两处都验)。**坑修 `0c4490a`**:首测撞 `DistlibException: Unable to locate finder for 'pip._vendor.distlib'`(pip 内置 distlib 按包 `__loader__` 类型查 resource finder,PyInstaller 冻结 loader 不在注册表)→ `sidecar_entry.py` 在 `--vc-pip` 分支跑 pip 前注册冻结 loader(传 loader **实例**,register_finder 内部 type();onedir 下 distlib data 物理在 `_internal`)。
- **🐛 GUI 真机抓出第二个坑(frozen-spawns-frozen,根因+修复已命令行钉死)**:用户在 win-unpacked app 点安装 → 卡死(子进程 11MB/0 CPU、py-extra 空)。排查:命令行 `--vc-pip` 真包下载完全正常(wheel 全匹配:llama-cpp win_amd64 / ctranslate2 cp312 / abetlen 索引 200)、openpipe 模拟 Electron stdin 也不卡 → 唯一差异 = **父进程是 frozen `core_rpc.exe`**。根因 = PyInstaller「frozen 进程 spawn 同款 frozen 进程」:子继承父的 `_PYI*`/`_MEIPASS2` bootloader env → 误判已在父冻结上下文 → 卡 bootstrap。**修 `runtime_extras._stream`**:frozen 时 spawn 子进程剥掉 `_PYI*`/`_MEIPASS2` env(+ `stdin=DEVNULL` 隔离,不继承 sidecar JSON-RPC stdin)。**临时自测钩子 `--vc-spawntest`**(frozen exe spawn 自己跑 install)命令行确认:修复后 `SPAWNTEST rc=0 six_installed=True`(已删钩子)。**这坑只有 frozen 父触发,命令行/普通 python 父/openpipe 全验不出 → GUI 终验的价值。**
- **打包态 E2E 命令行验证通过(本会话,完整 app 路径)**:起真 frozen `core_rpc.exe` server → 喂 `embedded_ai.install` RPC → job 线程 spawn frozen `--vc-pip` → 真包全装(faster-whisper/llama-cpp/ctranslate2 cp312/onnxruntime/av/numpy…)→ **`JOB succeeded {installed:True}`,py-extra 291MB,`faster_whisper/`+`llama_cpp/` 目录就位**,VC_USER_DATA 落位对。frozen-spawn 修复在 frozen 父+job 线程+真包 全条件下确认生效。(无害警告:pip 报冻结包内 pydantic 子依赖 metadata 不全,不影响 embedded AI 安装/运行。)
- **🐛 env 组件(yt-dlp 等)冻结态 detect+install 修复 + E2E(commit `62409df`,本会话补完)**:gpu/embedded-ai 之外,`core.env` 的普通 pip 组件(yt-dlp/openai/Pillow/google-genai/fish-audio-sdk)还漏改 —— ① detect 走 `importlib.metadata.version()`,但 base 冻结包内置这些**无 dist-info** → 误报「未安装」;② install 仍 `sys.executable -m pip` → 冻结态卡死(同根因)。修:**`detectors.detect_pip`** 加 importability fallback(metadata 命中=`source=pip`,否则 `find_spec` 命中=`source=bundled` version=None)+ `import_name` 参数(Pillow→PIL / google-genai→google.genai);**`pip_installer`** 改走 `runtime_extras`(`--vc-pip` 进 py-extra,`upgrade_pip`=`install_pip` 恒 `--upgrade`)。**冻结 E2E 决定性通过**(重打 sidecar + 干净 VC_USER_DATA):detect-before `source=bundled`/None → `env.install` 经 `--vc-pip` 进 py-extra 不卡 → detect-after `source=pip`/2026.3.17 → `py-extra/yt_dlp` 就位。dev venv detect 不变(`source=pip` 带版本)。pytest env+runtime_extras 绿。
- **✅ GUI 真机终验收口(2026-06-04)**:① env 组件 yt-dlp 冻结态 detect/install GUI 入口 —— 验中暴露**两个 UI 缺口**(已修 `d83d3b7`):(a) `detect_pip` 新 `source="bundled"` 漏 i18n 键 → 显示原始 key,补 `env.source.bundled`(随包/Bundled);(b) 已安装的可装组件**完全没按钮** → 内置 yt-dlp 无法升级(py-extra seam 的意义落空),改成 installable 恒显按钮(未装=安装/已装=更新,均跑 `env.install` 恒 `--upgrade` 进 py-extra)。② 嵌入 AI 安装 + ASR/LLM、③ CUDA nvidia 解析 —— 用户信任命令行 E2E 已钉死的机制(embedded-ai 291MB 进 py-extra + frozen-spawn 修复 + 同一 py-extra/nvidia 路径),未逐一手点。**重打包一次性出连贯冻结产物**(installer + win-unpacked,14:23;bundled sidecar 08:29 = env-fix build)。
- **下一步 = step 8 打磨**(图标/签名/ffmpeg pinned/CI)。commit `4ef3956`/`0c4490a`/`c5511fd`/`2a5e65d`/`bc0c672`/`62409df`/`d83d3b7` **未 push**,视用户意([[feedback_external_actions]])。
...
- **Step 4 收尾 ✅ 完成**:删 `MaterialWorkbench.tsx` + `SubtitlesTab.tsx`(+清 `workbenches/index.tsx` 死 registry);i18n 随各轮清 orphan(zh/en 475 对称);**清掉 Hub 死 `readiness` state**(state + `loadTree`/通知/`createMaterial` 的 `slot_readiness` fetch + `ProjectView` prop,均无消费者——SlotRow 已删、新侧栏自取 `slotReadinessStructured`)。**顺手修潜在 bug**:`createMaterial` 原 `setWorkbench({kind:"material"})` → `CreationWorkbench` 无 `news_video` 分发 = "not ported",改 `setMaterial(...)` 走侧栏视图(**待真机验**:新建素材应开侧栏,非 not-ported)。
