...
DEFAULTS: dict[str, str] = {
    ...
    "subtitle.pack": (
        ...
        "3. 每个 segment 的两个内容字段：\n"
        "   - refined：对该段内容做精炼总结，不超过 128 个汉字。"
        "对于问答段落，保留精炼后的问题与回答，并保持问答说话人的"
        "视角，不要改写为第三方转述。不要复述原文，给出信息密度高的概括。\n"
        "   - key_points：该段最重要的 3-5 条要点（数组），每条 ≤25 个汉字。"
        "**信息颗粒度要明显**：每条聚焦一个具体的事实、人物、数据或结论；"
        "禁止使用空泛的开场白、礼貌用语、过场词。\n"
        "     若该段内容确实贫瘠（如纯过场），可返回少于 3 条甚至空数组 []，"
        "不要凑数。\n"
        "\n"
    ...
    "news.source_context": (
        ...
        "\n"
        "你是新闻 / 时事类视频的资料分析助手。任务：从下面的少量种子"
        "信息出发，**主动用联网搜索能力**补全一份完整的事件档案，"
        "供下游 AI 任务（章节、标题、热点片段、发布稿、字幕烧录）"
        "使用。\n"
        "\n"
        "重要：种子信息有限，大部分字段都需要你通过搜索 URL 对应"
        "页面、查证主讲人当下职位、定位事件发生时间地点、检索相关"
        "历史背景来填充。不要凭训练数据记忆猜测，请用搜索交叉"
        "验证。\n"
        "\n"
        "## 输入种子\n"
        "\n"
        ...
        "\n"
        "### 用户已知信息 (锚定真相 + 已有上下文)\n"
        "下列字段是用户手工提供或之前已确认的事实。**你的推理必须"
        "与这些字段保持一致**；这些字段的值用户认为是真的，不要"
        "在你的输出中产生跟它们矛盾的内容。下面 JSON 中已有非空"
        "值的字段也不在你的输出 schema 里，不要尝试覆盖。\n"
        "{existing_filled}\n"
        "\n"
        "## 输出字段定义\n"
        "\n"
        "全部 10 个字段为 string 类型。**缺乏证据时返回空字符串 \"\"**，"
        "不要写 \"未知\" / \"unknown\" / \"N/A\" / \"待定\" 等占位。\n"
        "\n"
        "### 人物\n"
        ...
        "日期部分必须与用户提供的 event_date 一致；时分部分主动搜索"
        ...
        "覆盖 \"谁在哪里做了什么、要点是什么\"。须与用户提供的 host /"
        " event_date / event_location / episode_topic 保持一致\n"
        ...
        "2. 搜索主讲人当下职位、所属机构、近期动态\n"
        "3. 定位事件发生的具体日期、地点 (页面元数据 + 新闻报道"
        "交叉验证)\n"
        "4. 检索相关历史背景写入 background\n"
        "5. 综合以上信息撰写 event_summary 与 key_points\n"
        "\n"
